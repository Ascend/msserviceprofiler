...
The hardware environment supported by the tool is the same as that supported by MindIE. For details, see "Environment Preparation" in [MindIE Installation Guide](https://gitcode.com/Ascend/MindIE-Motor/blob/v3.1.0/docs/en/user_guide/environment_preparation.md).
...
4. After the preceding environment is prepared, you can use [msprechecker](https://gitcode.com/Ascend/msit/tree/26.1.0/msprechecker) to check environment variables and serving configurations.
...
MindStudio Insight can visualize the profile data ([Parsed Results](#parsed-results)) collected and analyzed by the service profiler. Currently, it supports visualizing `chrome\_tracing.json` and `profiler.db` files. For detailed operations and visualization results, see the section "[Serving Tuning](https://gitcode.com/Ascend/msinsight/blob/26.1.0/docs/en/user_guide/service_optimization.md)" in [MindStudio Insight User Guide](https://gitcode.com/Ascend/msinsight/blob/26.1.0/docs/en/user_guide/overview.md).
