...
configure_user_bin() {
    ...
    local marker="# msserviceprofiler-devcontainer-user-bin"
...
configure_python3() {
    ...
    for candidate in /opt/python/cp*-cp*; do
        if [ -d "$candidate/bin" ] && [ -x "$candidate/bin/python3" ]; then
            local marker="# msserviceprofiler-pyenv-python"
            for rcfile in "$HOME/.bashrc" "$HOME/.bash_profile"; do
                if [ -f "$rcfile" ] && ! grep -qF "$marker" "$rcfile"; then
                    cat >> "$rcfile" <<EOF
$marker
...
append_dev_hint_once() {
    local marker="# msserviceprofiler-dev-hint"
    local rcfile="$HOME/.bashrc"
    if grep -qF "$marker" "$rcfile" 2>/dev/null; then return; fi
    cat >> "$rcfile" <<EOF
$marker
# msserviceprofiler development commands:
...
log "Starting container initialization (msserviceprofiler)..."
configure_user_bin
configure_python3
install_build_deps
sync_git_identity
